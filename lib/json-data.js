...
const currentLatestPostList = require(`${process.cwd()}/data/latest-posts.json`);
...
function generateLastestPostList(fileName) {
  ...
  try { 
    ...
    if (!inArray(currentLatestPostList, title)) {
      ...
      fs.writeFileSync(`${process.cwd()}/data/latest-posts.json`, JSON.stringify(currentLatestPostList));
    }
  } catch (err) {
    console.log(err.message);
  }
