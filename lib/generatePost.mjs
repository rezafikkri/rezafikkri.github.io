...
async function generateSiteMap(slug, lastmod) {
  ...
  // parse back sitemapParsed to xml
  const builder = new Builder();
  const newSitemap = builder.buildObject(sitemapParsed);

  // write file to sitemap.xml
  writeFileSync(fullPath, newSitemap);
}
...
async function generatePost() {
  ...
  generateSiteMap(generatedSlug, dayjs(date).format('YYYY-MM-DD'));
}
