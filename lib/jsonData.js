...
const currentLatestPostList = require(`${process.cwd()}/data/latestPostList.json`);
...
function generateLastestPostList(fileName) {
  ...
  try { 
    ...
    if (!inArray(currentLatestPostList, title)) {
      ...
      fs.writeFileSync(`${process.cwd()}/data/latestPostList.json`, JSON.stringify(currentLatestPostList));
    }
  } catch (err) {
    console.log(err.message);
  }
